...
class MarklightTests: XCTestCase {
    ...
    func testAtxH1() {
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 0, effectiveRange: &range!) as? UIColor {
    ...
    func testAtxH2() {
        ...
        if let attribute = self.textStorage.attribute(.font, at: 2, effectiveRange: &range!) as? UIFont {
    ...
    func testReferenceLinks() {
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 8, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 2)
        } else {
            XCTFail()
    ...
    func testImage() {
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 9, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            // TODO: exetend test
            XCTAssert(range?.length == 2)
        } else {
    ...
    func testCodeBlock() {
        ...
        if let attribute = self.textStorage.attribute(.font, at: 3, effectiveRange: &range!) as? UIFont {
            let textSize = UIFontDescriptor.preferredFontDescriptor(withTextStyle: UIFontTextStyle.body).pointSize
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 25, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 3)
        } else {
            XCTFail()
    ...
    func testIndentedCodeBlock() {
        ...
        if let attribute = self.textStorage.attribute(.font, at: 0, effectiveRange: &range!) as? UIFont {
            let textSize = UIFontDescriptor.preferredFontDescriptor(withTextStyle: UIFontTextStyle.body).pointSize
            XCTAssert(attribute == UIFont(name: "Menlo", size: textSize))
            XCTAssert(range?.length == 33)
        } else {
    ...
    func testCodeSpan() {
        ...
        self.textStorage.replaceCharacters(in: NSMakeRange(0, 0), with: attributedString)
        var range : NSRange? = NSMakeRange(0, 1)
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 29, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 1)
        } else {
            XCTFail()
        }
        if let attribute = self.textStorage.attribute(.font, at: 30, effectiveRange: &range!) as? UIFont {
            let textSize = UIFontDescriptor.preferredFontDescriptor(withTextStyle: UIFontTextStyle.body).pointSize
            XCTAssert(attribute == UIFont(name: "Menlo", size: textSize))
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 34, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 1)
        } else {
            XCTFail()
    ...
    func testQuote() {
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 24, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 2)
        } else {
            XCTFail()
    ...
    func testItalic() {
        ...
        if let attribute = self.textStorage.attribute(.font, at: 1, effectiveRange: &range!) as? UIFont {
            let textSize = UIFontDescriptor.preferredFontDescriptor(withTextStyle: UIFontTextStyle.body).pointSize
        ...
        if let attribute = self.textStorage.attribute(.foregroundColor, at: 7, effectiveRange: &range!) as? UIColor {
            XCTAssert(attribute == UIColor.lightGray)
            XCTAssert(range?.length == 1)
        } else {
            XCTFail()
